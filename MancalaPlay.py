...
class Play:
    ...
    def computer_turn(self, Game: State, Noeud: Node, current_player, MCTS=False, DEPTH=6, heuristic=1, MINMAX=False):
        if len(Game.possible_moves(current_player)) > 0:
            if MINMAX:
                best_node, STEP = Search.MiniMaxAlphaBeta(
                    Noeud, DEPTH, current_player, Noeud.alpha, Noeud.beta, MCTS, heuristic)
